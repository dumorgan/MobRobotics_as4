//action_service
//gets the poses from action_client and send the cmd_vel to gazebo, based off of the lidar reads
...
bool lidar_alarm = false; //variable for the lidar alarm
...
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 ...
 dist = sqrt(goal_pose.position.x*goal_pose.position.x + goal_pose.position.y*goal_pose.position.y); //norm 2 euclidian metric
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation); 
 	...
 	//deltaPose.orientation.x = goal_pose.orientation.x - current_pose.orientation.x;
 	//deltaPose.orientation.y = goal_pose.orientation.y - current_pose.orientation.y;
 	//deltaPose.orientation.z = goal_pose.orientation.z - current_pose.orientation.z;
 	//deltaPose.orientation.w = goal_pose.orientation.w - current_pose.orientation.w;
...
bool callback(ps4_pkg::PathSrvRequest& request, ps4_pkg::PathSrvResponse& response) {
...
void alarmCallback (const std_msgs::Bool& alarm_msg) {
	if (alarm_msg.data) {
		ROS_INFO("ALARM RECEIVED, HALT MOVEMENT");
		lidar_alarm = true;
	}
	else {
		lidar_alarm = false;
...
{
  ros::init(argc, argv, "action_service");
  ros::NodeHandle n;
  ...
  ros::ServiceServer service = n.advertiseService("action_server", callback);
